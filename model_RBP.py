...
#tf.train.latest_checkpoint
    ...
    def build_model(self):
        ...
        with tf.variable_scope('gru_layer'):  
            ...
            #saved_states = []
            #saved_states.append(list(state))
...
#        '''
 #       Try different optimizers.
  ...
  # Implementing RBP
        ...
        state_last = output
        state_2nd_last = output[-2]
        grad_state_last = tf.gradients(self.cost, output)
        update_forward_diff = GRUCell(self.rnn_size, activation=self.hidden_act)
        gvs = RBP1.RBP(params_dynamic=params_dynamic,
                  state_last = state_last,
                  state_2nd_last = state_2nd_last,
                  grad_state_last = grad_state_last,
                  update_forward_diff = update_forward_diff,
                  eta=1.0e-5,
                  truncate_iter=50,
                  rbp_method='CG_RBP')
...
# First event of each session. 
            while not finished:
                ...
                for i in range(minlen-1):
                    ...
                    fetches = [self.cost, self.final_state, self.output, self.global_step, self.lr, self.train_op]
                    # input/output
                    ...
                    cost, state, output, step, lr, _ = self.sess.run(fetches, feed_dict)
